...
//! 9. Install `IComponentHandler` so GUI edits reach `process()`.
//!
...
/// Linux/Windows Steinberg module entry — must run before GetPluginFactory.
/// yabridge's chainloader asserts if this is skipped.
...
/// enough. Linux additionally requires `ModuleEntry` before
/// `GetPluginFactory` (Steinberg SDK / yabridge).
#[cfg(not(target_os = "macos"))]
struct LoadedModule {
    library: libloading::Library,
    /// True when we successfully called `ModuleEntry` (so Drop
    /// should call `ModuleExit`).
    entered: bool,
    path: PathBuf,
}
...
impl LoadedModule {
    unsafe fn open(bundle: &Path) -> Result<Self> {
        ...
        // Steinberg Linux VST3 module API: ModuleEntry before factory.
        let entered = match unsafe {
            library.get::<unsafe extern "C" fn(*mut std::ffi::c_void) -> bool>(MODULE_ENTRY_SYMBOL)
        } {
            Ok(entry) => {
                ...
                if !ok {
                    return Err(Error::LoadFailed {
                        path: bundle.to_path_buf(),
                        reason: "ModuleEntry returned false".into(),
                    });
                }
                true
            }
            // Older plugins may omit ModuleEntry and init in GetPluginFactory.
            Err(_) => false,
        };
...
pub struct Vst3Plugin {
    ...
    /// Host application + Linux run-loop (must outlive the plugin).
...
impl Vst3Plugin {
    /// Pump Linux `IRunLoop` timers and FD handlers (call from the UI thread).
    pub fn pump_host_services(&self) {
    ...
    fn load_from(info: &PluginInfo) -> Result<Self> {
        ...
        // Initialize with host context (IHostApplication + Linux IRunLoop).
        ...
        let (controller, separate_controller) = if cls_id_status == kResultTrue {
            ...
            if unsafe { ctrl_ptr.initialize(host_ctx) } != kResultOk {
                return Err(Error::LoadFailed {
                    path: info.path.clone(),
        ...
        let (pending_params, component_handler) = host::install_component_handler(&controller)
            .map_err(|status| Error::LoadFailed {
                path: info.path.clone(),
                reason: format!("setComponentHandler failed ({status})"),
            })?;
...
impl PluginCore for Vst3Plugin {
    ...
    fn parameter_value_string(&self, index: usize, value: f64) -> Result<String> {
        ...
        if unsafe {
            self.controller
                .getParamStringByValue(info.id, clamped, &raw mut string)
        } != kResultOk
        {
            return Err(Error::Other(
                "IEditController::getParamStringByValue failed".into(),
            ));
        }
    ...
    fn set_parameter(&mut self, index: usize, value: f64) -> Result<()> {
        ...
        // Also enqueue for the audio processor (required for separate
        // component/controller plugins).
        self.pending_params.push(info.id, clamped);
        Ok(())
    }
...
impl Plugin<f32> for Vst3Plugin {
    fn process(
        ...
        // Reflect processor → controller param updates (meters, etc.).
        host::apply_output_param_changes(&self.controller, &output_params);
